...
启动后端（可配合 `SERVE_FRONTEND=true` 单进程访问 <http://127.0.0.1:8000>），按发布向导三步走：
...
| 3 选主题与预览发布 | 左侧选主题卡（默认 default），右侧手机框即时渲染主题预览（与最终发布渲染一致，含配图/封面/作者）；预览默认只读，点【编辑】可原位修改 Markdown（完成编辑后重渲染）→ 确认发布 | 未选主题时发布按钮禁用；成功态展示真实 media_id；渲染约 1~3s（加载态覆盖） |
| 4 核对草稿箱 | mp.weixin.qq.com → 内容与互动 → 草稿箱（新的创作/图文消息） | 标题/封面/作者/配图/排版与预览一致 |
| 5 查看记录 | 系统「发布记录」页 → 查看快照 | 成功记录 + media_id + 内容快照可回看 |

> 主题预览依赖本机已安装 wenyan-mcp（本地 node + `@wenyan-md/core` 渲染，无需微信凭据、不外呼）；未安装时报 `PUBLISH_MCP_NOT_INSTALLED`，按 §3 安装即可。
...
| `PUBLISH_RENDER_ERROR`（主题预览） | 主题预览渲染失败（主题不存在 / node 脚本异常） | 换主题重试；仍失败则核对 wenyan-mcp 安装 |
| `PUBLISH_RENDER_TIMEOUT`（主题预览） | 渲染子进程超 30s（含 mermaid/公式的长文首渲染较慢） | 点「重试」重新渲染 |
