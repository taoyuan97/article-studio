...
- **`article_agent/registry.py`**：`ModelCapabilities.structured_output_method` 集中声明 Provider 的结构化输出方式；DeepSeek 使用 `json_mode`，Moonshot 使用 `function_calling`。`get_structured_chat_model(...)` 是业务层统一入口，避免业务服务散落 Provider 条件判断。
- **`app/plan_service.py`**：`generate_image_plan(...)` 同步编排——解析文章版本（`version_id` 空则取 `current_version_id`，无版本 → `PLAN_NO_CONTENT`）→ 组装输入（全文 + `split_sections` 章节数 + `split_blocks` 块编号清单（`{index, kind, preview}`）+ 后端计算字数）→ `registry.get_structured_chat_model(provider, model, ImagePlanResult, include_raw=True)` 一次调用 → 校验（空 JSON content → 明确 `PLAN_LLM_ERROR`；Pydantic 解析失败 → `PLAN_LLM_ERROR`；`block_index` clamp 到 [1, 块总数] 并按位置去重保留首个；`images` 空 → `PLAN_EMPTY`）→ 覆盖式持久化 `image_plans` 并返回完整载荷（GET 原样回放，刷新可恢复）。
