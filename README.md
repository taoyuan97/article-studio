...
| 发布到公众号 | 四步向导（选文章/版本 → 正文画布锚点定位插图 → 选主题 → 预览编辑）一键推送至个人公众号草稿箱（仅建草稿不群发），发布记录与内容快照可回看 |
...
- **发布到公众号**：文章工作台右上「发布到公众号」进入四步向导——版本和信息（文章/版本 + 「选择封面」弹窗从全部图片单选 + 作者）→ 配图与位置（正文画布点击任意段落/标题设置插入点，「插入配图」弹窗多选（本文配图置顶 + 素材库图片），确定后按勾选顺序插入；已插图内联展示、hover 可删除）→ 选排版主题 → 预览组装 Markdown（可直接编辑）→ 确认发布到公众号草稿箱（仅建草稿，不群发；微信要求封面或正文至少一张图，封面可选不插入正文的图）。
